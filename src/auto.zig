...
fn windowsTaskMatches(allocator: std.mem.Allocator, codex_home: []const u8, self_exe: []const u8) !bool {
    ...
    const task_xml = windows_task_scheduler.readTaskXmlAlloc(allocator, windows_task_name) catch return false;
    const xml = task_xml orelse return false;
    defer allocator.free(xml);

    const actual_fingerprint = (try windowsTaskDefinitionFingerprintFromXml(allocator, xml)) orelse return false;
...
fn windowsExpectedTaskDefinitionFingerprint(allocator: std.mem.Allocator, action: []const u8) ![]u8 {
    return windowsTaskDefinitionFingerprint(
        allocator,
        action,
        windows_task_trigger_kind,
        windows_task_restart_count,
        windows_task_restart_interval_xml,
        windows_task_execution_time_limit_xml,
    );
}

fn windowsTaskDefinitionFingerprint(
    allocator: std.mem.Allocator,
    action: []const u8,
...
fn windowsTaskDefinitionFingerprintFromXml(allocator: std.mem.Allocator, xml: []const u8) !?[]u8 {
    ...
    return try windowsTaskDefinitionFingerprint(
        allocator,
        action,
        trigger_kind,
        restart_count,
